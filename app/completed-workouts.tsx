    ...
    return (
        <ScrollView style={styles.screenContainer}>
            <VStack style={styles.container}>
                {logs?.map(log => (
                    <View key={log.mesoId}>
                        <Text onPress={() => handleOpenLog(log.mesoId)} style={styles.title}
                              key={log.mesoId}>{log.mesoTitle}</Text>
                        <View>
                                                            ...
                                                            <HStack space={10}>
                                                                {week.workouts.map((workout, i) => (
                                                                    <VStack style={styles.day} key={i}>
                                                                        <Text
                                                                            style={styles.dayName}>{Days[workout.day].toUpperCase()}</Text>
                                                                        <View style={styles.workout} key={i}>
                                                                            {workout.exercises.map(exercise => (
                                                                                <View key={exercise.id}>
                                                                                    <Text
                                                                                        style={[styles.whiteText, styles.exerciseTitle, {marginBottom: 5}]}>{exercise.exercise}</Text>
                                                                                    <HStack marginLeft={5}>
                                                                                        <VStack space={5}>
                                                                                            <Text
                                                                                                style={styles.whiteText}>Reps</Text>
                                                                                            <Text
                                                                                                style={styles.whiteText}>Weight</Text>
                                                                                        </VStack>
                                                                                        <HStack
                                                                                            style={{marginLeft: 30}}>
                                                                                            {exercise.data.map((data, i) => (
                                                                                                <VStack key={i}
                                                                                                        ...
                                                                                                        space={5}>
                                                                                                    <Text
                                                                                                        style={styles.whiteText}>{data.reps}</Text>
                                                                                                    <Text
                                                                                                        style={[styles.whiteText, {marginBottom: 20}]}>{data.weight}</Text>
                                                                                                </VStack>
                                                                                            ))}
                                                                                        </HStack>
                                                                                    </HStack>
